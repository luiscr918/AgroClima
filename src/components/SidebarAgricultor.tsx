...
interface SidebarAgricultorProps {
  ...
  usuario: Usuario; // viene del useAuth (id, email, rol)
  onCerrarSesion: () => void;
}
...
export const SidebarAgricultor = ({
  ...
  // Estado para guardar el usuario completo desde la BD
  const [usuarioCompleto, setUsuarioCompleto] = useState<Usuario>();

  // Cargar datos completos al montar el componente
  useEffect(() => {
    const fetchUser = async () => {
      try {
        if (usuario?.id) {
          const res = await UsuarioService.getUsuarioById(usuario.id);
  ...
  const handleCrearTerreno = () => {
    navigate("/nuevo-terreno");
  };
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-3 py-6 space-y-2 overflow-y-auto">
        ...
        >
          <Home className="w-5 h-5 shrink-0 group-hover:text-green-300" />
          {sidebarOpen && (
            <span className="text-left text-sm font-medium">Inicio</span>
          )}
        </motion.button>
        ...
        <motion.button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-gray-300 hover:text-white hover:bg-green-700 group"
        >
          <Plus className="w-5 h-5 shrink-0 group-hover:text-green-300" />
          {sidebarOpen && (
            <span className="text-left text-sm font-medium">Nuevo Terreno</span>
          )}
        </motion.button>
        ...
        <motion.button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-gray-300 hover:text-white hover:bg-green-700 group"
        >
          <MapPin className="w-5 h-5 shrink-0 group-hover:text-green-300" />
          {sidebarOpen && (
            <span className="text-left text-sm font-medium">Mis Terrenos</span>
          )}
        </motion.button>
        ...
        <motion.button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-gray-300 hover:text-white hover:bg-green-700 group"
        >
          <User className="w-5 h-5 shrink-0 group-hover:text-green-300" />
          {sidebarOpen && (
            <span className="text-left text-sm font-medium">Perfil</span>
          )}
        </motion.button>
      </nav>
      ...
      <div className="border-t border-green-700 bg-green-900 bg-opacity-50 space-y-3 p-4">
        ...
        <motion.button
          ...
          className="w-full flex items-center justify-center gap-2 px-3 py-2 text-gray-300 hover:text-white hover:bg-green-700 rounded-lg transition"
        >
